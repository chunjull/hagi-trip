...
- [x] `P1 UI` 顯示 collaboration 的 `UPCOMING`、`ACTIVE`、`ENDED` 狀態。
- [x] `P1 UI` 在 JR 萩駅／東萩駅 Detail 顯示「〇〇のはなし」時段。
- [x] `P1 UI` 顯示住宿合作日，但不套用即時營業 badge。
- [x] `P1 UI` 顯示「コラボ協力店舗」badge 與共同特典資訊。
- [x] `P1 UI` 正確呈現機場商店分段營業、松陰神社個別內容與着物返却時間。
- [x] `P1 DOCS` 完成 `/about`：用途、使用方式、時區、來源與更新原則。
- [x] `P1 DOCS` 加入非官方網站、SNS／臨時休業不納入及固定免責聲明。
- [x] `P1 DOCS` 加入開發背景與開發者聯絡方式。
- [x] `P1 UI` 完成共用 Header 與 mobile Bottom Navigation。
