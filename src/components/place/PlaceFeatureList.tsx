...
import { FEATURE_KIND_LABELS, formatAvailabilityDateRanges } from "@/components/place/feature-display";
...
const PlaceFeatureList = ({ businessInfoSuppressed = false, date, features }: PlaceFeatureListProps) => (
  <ul className="space-y-3">
    {features.map((feature) => {
      ...
      const shouldShowSchedule = !businessInfoSuppressed && feature.schedule !== undefined && isFeatureDateApplicable(feature, date);
      ...
      return (
        <li className="rounded-2xl border border-slate-200 bg-white p-4" key={feature.id}>
          ...
          {feature.availabilityDates !== undefined ? (
            ...
            <p className="mt-3 rounded-xl bg-amber-50 px-3 py-2 text-xs leading-5 text-amber-950">
              官方活動資料未列出此方案的指定住宿日，預訂前請向住宿設施確認。
            </p>
          ) : null}
          ...
          {collaborationState === "ACTIVE" && availabilityDateState === "NOT_LISTED" ? (
            <p className="mt-3 text-sm leading-6 text-slate-600">此聯名內容不在所選日期提供。</p>
          ) : null}
